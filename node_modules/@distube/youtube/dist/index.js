...
var playError = require("@distube/ytdl-core/lib/utils").playError;
var YouTubePlugin = class extends import_distube.ExtractorPlugin {
  ...
  async getStreamURL(song) {
    ...
    const format = info.formats.filter((f) => f.hasAudio && (newSong.duration < 10 * 60 || f.hasVideo) && (!newSong.isLive || f.isHLS)).sort((a, b) => Number(b.audioBitrate) - Number(a.audioBitrate) || Number(a.bitrate) - Number(b.bitrate))[0];
    if (!format) throw new import_distube.DisTubeError("UNPLAYABLE_FORMATS");
    return format.url;
  }
  async getRelatedSongs(song) {
    return (song.related ? song.related : (await import_ytdl_core.default.getBasicInfo(song.url, this.ytdlOptions)).related_videos).map(
